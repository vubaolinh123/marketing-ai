...
export default function ImageDropzone({ images, onChange, disabled, maxImages = 1 }: ImageDropzoneProps) {
    ...
    }, [onChange, disabled, hasMaxImages, isMultiMode, maxImages, images]);
    ...
    return (
        <div className="space-y-4">
            {/* Dropzone - hidden when max reached */}
            {!hasMaxImages && (
                <div
                ...
                >
                    ...
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs text-gray-400">
                            {isMultiMode
                                ? `PNG, JPG, WEBP (tối đa 10MB/ảnh) — ${images.length}/${maxImages} ảnh`
                                : 'PNG, JPG, WEBP (tối đa 10MB) - Chỉ 1 ảnh nguồn, AI sẽ tạo nhiều góc'}
                        </p>
                    </div>
                </div>
            )}
            ...
            {images.length > 0 && (
                <div className={cn(
                    ...
                    {/* Add more images placeholder in multi mode */}
                    {isMultiMode && !hasMaxImages && (
                        <label className="relative aspect-square rounded-xl border-2 border-dashed border-gray-300 hover:border-amber-400 bg-gray-50 hover:bg-amber-50 flex flex-col items-center justify-center cursor-pointer transition-all">
                            <input
                                type="file"
                                accept="image/*"
                                multiple
